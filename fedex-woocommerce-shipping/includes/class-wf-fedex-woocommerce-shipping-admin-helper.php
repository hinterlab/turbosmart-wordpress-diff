		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			foreach( $additional_products as $values) {
				...
				$group['InsuredValue'] = array(
					'Amount'   => $this->convert_to_fedex_currency(round( $this->wf_get_insurance_amount($values['data']) )),
	...
	private function xa_get_box_packages( $packages, $group_id=1 ){
		...
		foreach ( $packages as $package ) {
			...
			$group = array(
				...
				'InsuredValue'      => array(
					'Amount'   => $this->convert_to_fedex_currency(round( $boxinsuredprice )),
					'Currency' => $this->wf_get_fedex_currency()
				),
			...
			foreach ( $fedex_packages as $key => $parcel ) {
					...
					if ( $parcel_request['packed_products'] ) {
						foreach ( $parcel_request['packed_products'] as $product ) {
							...
							if ( isset( $commodoties[ $product->id ] ) ) {
								...
								$commodoties[ $product->id ]['CustomsValue']['Amount'] += $this->wf_get_insurance_amount($product);
								continue;
							}
							$commodoties[ $product->id ] = array(
								...
								'UnitPrice'            => array(
									'Amount'           => $this->convert_to_fedex_currency($this->get_product_price($product)),
									'Currency'         => $this->wf_get_fedex_currency()
								),
								'CustomsValue'         => array(
				...
				if ( 'freight' === $request_type ) {
					...
					if( (isset($_GET['cod']) && $_GET['cod'] === 'true' ) || ( ! isset($_GET['cod'])  && $order_payment_method == 'cod' ) ){
						...
						$request['RequestedShipment']['SpecialServicesRequested']['CodDetail']['CodCollectionAmount']['Amount'] = $order_total ? $this->convert_to_fedex_currency($order_total) : $parcel_value;
				...
				if ( 'freight' === $request_type ) {
					...
					$request['RequestedShipment']['FreightShipmentDetail']['LineItems'] = array(
						...
						'Packaging'    => 'SKID',
						'Description' => ! empty($this->settings['item_description']) ? $this->settings['item_description'] : 'Heavy Stuff',
						'Weight'       => array(
							'Units'    => $this->labelapi_weight_unit,
				...
				if ($this->origin_country !== $package['destination']['country'] || !in_array($this->origin_country,array('US'))) {
					...
					$request['RequestedShipment']['CustomsClearanceDetail']['CustomsValue'] = array('Amount' => $this->convert_to_fedex_currency($this->order->get_total()), 'Currency' => $this->wf_get_fedex_currency());
			...
			foreach( $group_index_package_index as $packages_indexes ) {
				...
				if( ! empty($packages_indexes) ) {
					...
					if( ! empty($packages[$main_arr_index][$inner_arr_index]) ) {
						...
						$new_packages[$main_arr_index][$inner_arr_index]['InsuredValue']['Amount'] = array_shift($temp_insurance_arr);
					}
				}
			}
		...
		if ( $result->HighestSeverity != 'FAILURE' && $result->HighestSeverity != 'ERROR' && ! empty ($result->CompletedShipmentDetail) ) {
			...
			if(property_exists($result->CompletedShipmentDetail,'CompletedPackageDetails')){
				...
				if(property_exists($result->CompletedShipmentDetail->CompletedPackageDetails,'PackageDocuments')){
					...
					// For OP900 label, All the things related to OP900 has been handled in addon
					// Addon link - https://bitbucket.org/varun0502/fedex-addon/src/master/
...
//						To print the additional label(like OP900) while creating the shipment label only
//						header('Content-Type: application/PDF');
//						print($package_documents->Parts->Image);die;
					}
				}
	...
	public function wf_get_parcel_details($request){
	 ...
	 public function result_notifications( $notes, $error_message = '' ){
        $error_message = '';
	...
	private function manual_dimensions( $package ) {
		...
		$group =array(
			array(
				...
				'InsuredValue' => array(
					'Amount'   => $this->convert_to_fedex_currency(round( $total_price )),
					'Currency' => $this->wf_get_fedex_currency()
				)
			)
		);
