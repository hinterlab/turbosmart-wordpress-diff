...
class wf_fedex_woocommerce_shipping_method extends WC_Shipping_Method {
	...
	public function residential_address_validation( $package ) {
		...
		if ( !empty($package['destination']['address']) && !empty($package['destination']['country']) ) {
			...
			foreach ( $parcel_chunks as $parcels ) {
						...
						if ( $parcel_request['packed_products'] ) {
							...
							foreach ( $parcel_request['packed_products'] as $product ) {
								...
								if ( isset( $commodoties[ $product->id ] ) ) {
									...
									$commodoties[ $product->get_id() ]['CustomsValue']['Amount'] += $this->wf_get_insurance_amount($product);
									continue;
								}
								...
								$hst = get_post_meta( $product->get_id() , '_wf_hs_code', 1);
								if( ! empty($hst) ) {
				...
				if ( 'freight' === $request_type ) {
					...
					foreach ($request['RequestedShipment']['RequestedPackageLineItems'] as $key => $item) {
						if( isset($item['Dimensions']) && max( array_map( array($this,'dimensions_in_inches'), array($item['Dimensions']['Length'], $item['Dimensions']['Width'], $item['Dimensions']['Height']) ) )  > 180 ){
					...
					if( WC()->customer->get_meta( 'xa_fedex_lift_gate_for_delivery', true) ){
					...
					if( WC()->customer->get_meta( 'xa_fedex_inside_delivery', true) ){
	...
	*/
	...
	private function process_result( $result = '' ) {
			...
			foreach ( $rate_reply_details as $quote ) {
				...
				if($this->delivery_time) {
					...
					if( !empty($delivery_time_details) ){
						...
						if( $quote->ServiceType == 'FEDEX_GROUND' || $quote->ServiceType == 'SMART_POST' ) {
							...
							while( $transit_time_as_num != 0 ) {
								...
								if( $updated_date->format('D') == 'Sat' && $quote->ServiceType != 'SMART_POST'){
									$updated_date->modify("+2 day");
								}
								elseif( $updated_date->format('D') == 'Sun' ) {
									$updated_date->modify("+1 day");
								}
							}
		...
		elseif( is_admin() && ! is_ajax() && ! empty($result->HighestSeverity) && ! empty(! empty($result->Notifications) ) ){
	...
	public function convert_to_store_currency($rate_cost) {
		if( $this->fedex_currency != $this->wc_store_currency && ! empty($this->fedex_conversion_rate) ) {
			$rate_cost = $rate_cost / $this->fedex_conversion_rate;
		}
		return $rate_cost;
	}
	...
	public function add_found_rates() {
		if ( $this->found_rates ) {
			...
			if( ! empty($this->min_shipping_cost) ) {
				foreach ( $this->found_rates as $key => $rate ) {
					if( (double) $this->found_rates[ $key ][ 'cost' ] < (double) $this->min_shipping_cost ) {
						$this->found_rates[ $key ][ 'cost' ] = (double) $this->min_shipping_cost;
					}
				}
			}
